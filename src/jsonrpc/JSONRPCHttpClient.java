...
public class JSONRPCHttpClient extends JSONRPCClient
{
	...
	public JSONRPCHttpClient(HttpClient cleint, String uri){
        ((AbstractHttpClient) cleint).setKeepAliveStrategy(new ConnectionKeepAliveStrategy() {
            @Override
		...
		try
		{
			...
			if (jsonResponse.has("error"))
			{
				...
				if (!jsonError.equals(null))
					throw new JSONRPCException(jsonResponse.get("error") + " error_code:" + jsonResponse.get("error_code") + " error_msg:" + jsonResponse.get("error_msg"));
